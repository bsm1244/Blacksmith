...
void Memory::allocate_memory(size_t mem_size) {
  ...
  initialize_inv();
  // initialize(DATA_PATTERN::ZEROES);
}
...
void Memory::initialize(DATA_PATTERN data_pattern) {
  ...
  auto start_mem = DRAMAddr((char*) start_address);
  start_mem.set_physmap((char*) start_address);
  Logger::log_info("Set address map end.");
}

void Memory::initialize_inv() {
  Logger::log_info("Initializing memory with inverse sequence.");
  // uint fill_value = 0x33333333;
  // uint fill_value_inv = 0xcccccccc;
  uint fill_value = 0xffff0000;
  uint fill_value_inv = 0x0000ffff;
  // for each page in the address space [start, end]
  for (uint64_t cur_page = 0; cur_page < size; cur_page += getpagesize()) {
    // reseed rand to have a sequence of reproducible numbers, using this we can compare the initialized values with
    // those after hammering to see whether bit flips occurred
    // auto row = (((size_t)start_address + cur_page) >> 14) % 2; // 1 rank
    auto row = (((size_t)start_address + cur_page) >> 15) % 2; // 2 rank
    
    if(row == 0){
      for (uint64_t cur_pageoffset = 0; cur_pageoffset < (uint64_t) getpagesize(); cur_pageoffset += sizeof(int)) {
        uint64_t offset = cur_page + cur_pageoffset;
        auto p = (size_t)start_address + offset;
        *((int *) (p)) = fill_value;
      }
    }
    else{
...
size_t Memory::check_memory_sb(const volatile char *start, const volatile char *end, int bank_counter) {
  flipped_bits.clear();
  // create a "fake" pattern mapping to keep this method for backward compatibility
  PatternAddressMapper pattern_mapping;
  return check_memory_internal_sb(pattern_mapping, start, end, false, true, bank_counter);
}
...
size_t Memory::check_memory_internal(PatternAddressMapper &mapping,
  ...
  for (uint64_t i = start_offset; i < end_offset; i += pagesize) {
    ...
    for (uint64_t j = 0; j < (uint64_t) pagesize; j += sizeof(int)) {
      ...
      const auto cur_addr_dram = DRAMAddr((void *) (start_address + offset));
...
size_t Memory::check_memory_internal_sb(PatternAddressMapper &mapping,
                                     const volatile char *start,
                                     const volatile char *end,
                                     bool reproducibility_mode,
                                     bool verbose,
  ...
  void *page_raw = malloc(pagesize);
  void *page_raw_inv = malloc(pagesize);
  if (page_raw == nullptr || page_raw_inv == nullptr) {
    Logger::log_error("Could not create temporary page for memory comparison.");
    exit(EXIT_FAILURE);
  }
  memset(page_raw, 0, pagesize);
  memset(page_raw_inv, 0, pagesize);
  int *page = (int*)page_raw;
  int *page_inv = (int*)page_raw_inv;

  for (size_t j = 0; j < (unsigned long) pagesize/sizeof(int); ++j)
    // page[j] = 0x33333333; // row == 0
    page[j] = 0xffff0000; // row == 0
  for (size_t j = 0; j < (unsigned long) pagesize/sizeof(int); ++j)
    // page_inv[j] = 0xcccccccc; // row == 1
    page_inv[j] = 0x0000ffff; // row == 1
  ...
  for (uint64_t i = start_offset; i < end_offset; i += pagesize) {
    ...
    uint64_t addr = ((uint64_t)start_address+i);
    // auto row = (addr >> 14) % 2; // 1 rank
    auto row = (addr >> 15) % 2; // 2 rank
    ...
    if(row == 0){
      ...
      if ((addr + pagesize) < ((uint64_t)start_address+size) && memcmp((void*)addr, (void*)page_inv, pagesize) == 0){
        continue;
      }
    }
    ...
    for (uint64_t j = 0; j < (uint64_t) pagesize; j += sizeof(int)) {
      ...
      const auto cur_addr_dram = DRAMAddr((void *) (cur_addr));
      if (cur_addr_dram.bank != bank_counter) continue;
      ...
      // if the bit did not flip -> continue checking next block
      int expected_rand_value = 0;
      if(row == 0) expected_rand_value = page[j/sizeof(int)];
      else expected_rand_value = page_inv[j/sizeof(int)];
      // fprintf(stderr, "written data: %x, expected_data: %x\n", *((int *) cur_addr), expected_rand_value);
      if (*((int *) cur_addr)==expected_rand_value)
        continue;
