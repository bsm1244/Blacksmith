...
[[maybe_unused]] void TraditionalHammerer::n_sided_hammer(Memory &memory, int acts, long runtime_limit) {
  ...
  while (get_timestamp_sec() < execution_limit) {
    ...
    for (size_t ba = 0; ba < 4; ba++) {
      ...
      DRAMAddr cur_next_addr(ba, dist(gen)%131072, 0);
      ...
      Logger::log_info("Aggressors' VAs");
      for(auto aggr : aggressors){
        Logger::log_info(
          format_string("%lx",(size_t)aggr));
      }
      ...
      memory.check_memory_sb((const volatile char *) 0x10000000000, (const volatile char *) 0x10600000000, ba);
    }
  }
}

[[maybe_unused]] void TraditionalHammerer::n_sided_hammer_coupled_row(Memory &memory, int acts, long runtime_limit, int mode, int num_rows) {
  Logger::log_info("Traditional Hammering start!");
  std::random_device rd;
  std::mt19937 gen(rd());
  std::uniform_int_distribution<size_t> dist(0, std::numeric_limits<size_t>::max());
